...
class CupDBManager:
    ...
    def create_database(self):
        try:
            ...
            logging.Exception('Failed to create databse: ' + str(sys.exc_info()))
            raise
    ...
    def insert_question(self,question):
        ...
        try:
            ...
            logging.Exception('Failed to insert question: '+ str(sys.exc_info()))
            raise
        finally:
            c.close()
    ...
    def insert_tag(self, tag_name, is_company=0):
        try:
            c = self.conn.cursor()
            c.execute('''INSERT OR IGNORE INTO cup_tag VALUES(?,?)''',(tag_name,is_company))
            # self.conn.commit()
        except Exception as e:
    ...
    def insert_question_with_tag(self, question_id, tag_name):
        try:
            c = self.conn.cursor()
            c.execute('''INSERT OR IGNORE INTO cup_question_with_tag VALUES(?, ?)''',(question_id, tag_name))
            # self.conn.commit()
        except Exception as e:
            logging.Exception('Failed to insert question_with_tag: '+ str(sys.exc_info()))
            raise
        finally:
            c.close()
