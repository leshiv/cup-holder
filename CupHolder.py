...
class Fetcher(threading.Thread):
    ...
    def run(self):
        global __cache__
        logging.debug("Fetcher" + "_" + threading.current_thread().name +
                      "_" + str(threading.current_thread().ident))
        while True:
                ...
                try:
                    ...
                    if isinstance(e, urllib2.HTTPError):
                        if e.code in [401, 403, 404, 501, 503]:
                            ...
                            logging.warn(
                                "Failed to fetch page: " + url + ", error code" + str(e.code))
                    ...
                    if url not in tempts:
                        ...
                        logging.warning(
                            "Failed to fetch page: " + url + " after " + str(CupConfig.retries) + " retries")
                        self.url_queue.task_done()
...
class Miner(threading.Thread):
    ...
    def run(self):
        logging.debug("Miner" + "_" + threading.current_thread().name +
                      "_" + str(threading.current_thread().ident))
        while True:
...
class Keeper(threading.Thread):
    ...
    def run(self):
        ...
        logging.debug("Keeper" + "_" + threading.current_thread().name +
                      "_" + str(threading.current_thread().ident))
    ...
    def save(self, question):
        ...
        for tag in question.tags:
            ...
            self._db_manager.insert_question_with_tag(
                inserted_question_id, tag)
...
def main():
    ...
    logging.basicConfig(
        format='[%(levelname)s]:%(message)s', level=CupConfig.logging_level)

    # set up proxy if necessary
    if CupConfig.socks_proxy is not None:
        socks.setdefaultproxy(
            socks.PROXY_TYPE_SOCKS5, CupConfig.socks_proxy[0], CupConfig.socks_proxy[1])
    ...
    urls = CareerCupParser.generate_target_urls(
        CupConfig.base_url, CupConfig.query_params, __cache__)
    for url in urls:
        url_queue.put(url)
